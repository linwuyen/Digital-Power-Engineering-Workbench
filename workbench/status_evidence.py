...
from .evidence_io import EvidenceRecord, detail_row, load_evidence_records
...
def _record_gate(record: EvidenceRecord, execution_sha: str) -> dict | None:
    gate = record.gate or ""
    result = record.result
    if gate not in GATES:
        return None
    if record.execution_sha != execution_sha:
        return None
    if result not in RESULT_MAP:
        return None
    status = RESULT_MAP[result]
    evidence = record.evidence_ref.strip() if record.evidence_ref else None
...
def evidence_details_for_sha(data_root: Path, execution_sha: str) -> list[dict]:
    return [
        detail_row(record)
        for record in load_evidence_records(Path(data_root))
        if record.execution_sha == execution_sha
    ]
...
def qualification_for_sha(
    ...
    for record in load_evidence_records(Path(data_root)):
        normalized = _record_gate(record, execution_sha)
        if not normalized:
            continue

        if record.source_type == "regression_history" and normalized["status"] == "PASS":
            evidence_golden_sha = record.golden_sha
